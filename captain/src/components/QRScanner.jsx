// src/components/QRScanner.jsx - FINAL CLEAN VERSION
...
const QRScanner = () => {
  ...
  useEffect(() => {
    ...
    // ALL FUNCTIONS INSIDE useEffect ✅
    ...
    const markIn = (subordinate, qrData) => {
      ...
      alert(`✅ ${subordinate.name} marked IN successfully!\n\n⏰ Time: ${timeStr}`);
      navigate('/dashboard');
    };
    ...
    const markOut = (subordinate, qrData) => {
      ...
      const employeeIndex = markedIn.findIndex((emp) => emp.id === subordinate.id);
  ...
  return (
    <div className="scanner-container">
      <div className="scanner-card">
        <div className={`scanner-header ${scanOperation === 'markOut' ? 'mark-out-header' : ''}`}>
          <h2>📷 Scan QR Code</h2>
          <p>
        ...
        <div className="selected-info">
          <p className="info-title">
            {scanOperation === 'markIn' ? 'Marking IN for:' : 'Marking OUT for:'}
          </p>
          <div className={`selected-subordinate ${scanOperation === 'markOut' ? 'mark-out-border' : ''}`}>
            <div className="selected-name">{selectedSubordinate.name}</div>
            <div className="selected-code">{selectedSubordinate.salaryCode}</div>
        ...
        <div className="scanner-instructions">
          ...
          <ul>
            ...
            <li>
              Expected salary code: <strong>{selectedSubordinate.salaryCode}</strong>
            </li>
            {scanOperation === 'markOut' && inTime && (
              <li>
                Employee was marked IN at: <strong>{new Date(inTime).toLocaleTimeString()}</strong>
              </li>
            )}
            <li>⚡ Location is tracked continuously - instant scan!</li>
          </ul>
        </div>
      </div>
    </div>
